...
@section('content')
<section class="bg-[#0006] text-white rounded-lg w-96 p-">
    ...
    <div class="card w-full max-w-sm">
            <form method="POST" action="{{ route('login') }}" class="card-body">
                ...
                <input type="text" class="input bg-black" name="email" placeholder="Email" value="{{old('email')}}" />
                @error('email')
                    <small class="text-error text-xs">{{ $message }}</small>
                @enderror

                <label class="label">Password</label>
                <input type="password" class="input bg-black" name="password" placeholder="Password" />
                @error('password')
                    <small class="text-error text-xs">{{ $message }}</small>
                @enderror
